...
ReactionLayout::ReactionLayout(BaseReaction& r, bool smart_layout)
    : bond_length(MoleculeLayout::DEFAULT_BOND_LENGTH), default_plus_size(1), default_arrow_size(2), preserve_molecule_layout(false), _r(r),
      _smart_layout(smart_layout), reaction_margin_size(DEFAULT_HOR_INTERVAL_FACTOR), atom_label_width(1.3f), layout_orientation(UNCPECIFIED),
      max_iterations(0)
{
}

ReactionLayout::ReactionLayout(BaseReaction& r, bool smart_layout, const LayoutOptions& options)
    : bond_length(MoleculeLayout::DEFAULT_BOND_LENGTH), default_plus_size(options.getMarginSizeInAngstroms()),
      default_arrow_size(MoleculeLayout::DEFAULT_BOND_LENGTH * 2), preserve_molecule_layout(false), _r(r), _smart_layout(smart_layout),
...
void ReactionLayout::_updateMetadata()
{
    ...
    {
        const float ptab = first_single_product ? reaction_margin_size : reaction_margin_size / 2;
        const float rtab = last_single_reactant ? reaction_margin_size : reaction_margin_size / 2;
...
void ReactionLayout::make()
{
    ...
    if (_r.catalystCount())
    {
        ...
        _pushSpace(line, default_arrow_size);
